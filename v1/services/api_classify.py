...
from v1.schemas.payloads import (
    ClassifyPayload, ModelResponse, ErrorDescription, Response
)
...
class ApiClassify(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            error_description = ErrorDescription(raised=type(e).__name__,
                                                 raisedOn="ApiClassify", message=str(e), code="400")
            response = Response(message="failed", data=None, error=error_description, version=self.cfg.version)
            return response.dict(), 400

        response = Response(message="success",
                            data=ModelResponse(pred_confidence=pred_confidence, pred_label=pred_label,
                                               model_version=self.cfg.model._model_meta.run_id),
                            error=None, version=self.cfg.version)
